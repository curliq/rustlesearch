...
module.exports = ({ orl, channelsPath }) => {
  return {
    fetchChannels() {
      return fs.readFile(channelsPath, "utf8").then((text) => new Set(text.trim().split("\n")));
    },
    async listLogs(channel) {
      await fs.ensureDir(orl);
      const paths = channel ? await fg(`${orl}/${channel}/*.txt*`) : await fg(`${orl}/*/*.txt*`);
      return paths.map((p) => {
    ...
    listCompressedLogs(channel) {
      return this.listLogs(channel).then((logs) => logs.filter((log) => log.compressed));
    },
